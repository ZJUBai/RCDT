...
def load_datasets(folder_path,task):
    task2dataset_name = {
        ...
        "OfflineAntCircle-v0": "SafetyAntCircle-v0-200-5728.hdf5",
        ...
        "OfflineBallRun-v0": "SafetyBallRun-v0-80-940.hdf5",
        "OfflineCarButton1Gymnasium-v0": "SafetyCarButton1Gymnasium-v0-250-2656.hdf5",
        "OfflineCarButton2Gymnasium-v0": "SafetyCarButton2Gymnasium-v0-300-3755.hdf5",
        "OfflineCarCircle1Gymnasium-v0": "SafetyCarCircle1Gymnasium-v0-250-1271.hdf5",
        "OfflineCarCircle2Gymnasium-v0": "SafetyCarCircle2Gymnasium-v0-400-940.hdf5",
        "OfflineCarGoal1Gymnasium-v0": "SafetyCarGoal1Gymnasium-v0-120-1671.hdf5",
        "OfflineCarGoal2Gymnasium-v0": "SafetyCarGoal2Gymnasium-v0-200-4105.hdf5",
        "OfflineCarPush1Gymnasium-v0": "SafetyCarPush1Gymnasium-v0-200-2871.hdf5",
        "OfflineCarPush2Gymnasium-v0": "SafetyCarPush2Gymnasium-v0-250-4407.hdf5",
        "OfflinePointButton1Gymnasium-v0": "SafetyPointButton1Gymnasium-v0-200-2268.hdf5",
        "OfflinePointButton2Gymnasium-v0": "SafetyPointButton2Gymnasium-v0-250-3288.hdf5",
        "OfflinePointCircle1Gymnasium-v0": "SafetyPointCircle1Gymnasium-v0-200-1098.hdf5",
        "OfflinePointCircle2Gymnasium-v0": "SafetyPointCircle2Gymnasium-v0-300-895.hdf5",
        "OfflinePointGoal1Gymnasium-v0": "SafetyPointGoal1Gymnasium-v0-100-2022.hdf5",
        "OfflinePointGoal2Gymnasium-v0": "SafetyPointGoal2Gymnasium-v0-200-3442.hdf5",
        "OfflinePointPush1Gymnasium-v0": "SafetyPointPush1Gymnasium-v0-150-2379.hdf5",
        "OfflinePointPush2Gymnasium-v0": "SafetyPointPush2Gymnasium-v0-200-3242.hdf5",
        # === Velocity Environments ===
        "OfflineAntVelocityGymnasium-v1": "SafetyAntVelocityGymnasium-v1-250-2249.hdf5",
        "OfflineHalfCheetahVelocityGymnasium-v1": "SafetyHalfCheetahVelocityGymnasium-v1-250-2495.hdf5",
        "OfflineHopperVelocityGymnasium-v1": "SafetyHopperVelocityGymnasium-v1-250-2240.hdf5",
        "OfflineSwimmerVelocityGymnasium-v1": "SafetySwimmerVelocityGymnasium-v1-200-1686.hdf5",
        "OfflineWalker2dVelocityGymnasium-v1": "SafetyWalker2dVelocityGymnasium-v1-300-2729.hdf5"
    }
    full_path = os.path.join(folder_path, task2dataset_name[task])
    return load_hdf5_to_memory(full_path)


def load_hdf5_to_memory(file_path):
    """
    一次性将整个 HDF5 文件加载到内存中，返回一个字典，结构与 env.get_dataset() 一致。
    
    Args:
        file_path (str): HDF5 文件路径，例如 'cqdt_augmented.h5'
    
    Returns:
        dict: 包含 'observations', 'actions', 'rewards', 'costs', 'terminals', 'timeouts' 等键的字典，
              所有值均为 numpy 数组。
    """
    data_dict = {}
    with h5py.File(file_path, 'r') as f:
        for key in f.keys():
            # 关键：使用 [:] 将整个 dataset 读入内存
            data_dict[key] = f[key][:]  # 这会触发实际的数据读取和复制
    return data_dict


def label_expert_trajectories(dataset, top_reward_pencent: float, cost_bins:int):
    # ---------------------------- 1. 计算每条轨迹的累积cost 与 reward ----------
    def traj_stats(traj):
        r = float(traj["returns"][0])
        c = float(traj["cost_returns"][0])
        return r, c

    rewards = []          # 每条轨迹的聚合 reward
    costs = []            # 每条轨迹的聚合 cost
    for traj in dataset:
        r, c = traj_stats(traj)
        rewards.append(r)
    ...
    # ---------------------------- 2. 按 cost 值域均分 num_grids 网格 -------------------
    num_grids = cost_bins
    c_min, c_max = min(costs), max(costs)
    bins = np.linspace(c_min, c_max, num_grids+1)      # num_grids + 1 个端点 → num_grids 区间
    digit = np.digitize(costs, bins) - 1      # 0~num_grids-1 的网格编号

    # ---------------------------- 3. 每网格取 reward 最高 10 % 轨迹 ------------
    expert_mask = np.zeros(len(dataset), dtype=bool)
    for grid_id in range(num_grids):
        idx_in_grid = np.where(digit == grid_id)[0]
        if idx_in_grid.size == 0:
            continue
        # 按 reward 降序
        top_k = max(1, int(np.ceil(top_reward_pencent * idx_in_grid.size)))
        best_idx = idx_in_grid[np.argsort(-np.array(rewards)[idx_in_grid])[:top_k]]
        expert_mask[best_idx] = True

    # ---------------------------- 4. 写回 is_expert 字段 ------------------------
    r,c=[],[]
    for traj, flag in zip(dataset, expert_mask):
        traj['is_expert'] = bool(flag)
        if flag:
            r.append(traj["returns"][0])
            c.append(traj["cost_returns"][0])

    # print(len(expert_trajs))
    # rew = [traj["returns"][0] for traj in dataset]
    # rew_expert = [traj["returns"][0] for traj in expert_trajs]
    # cost = [traj["cost_returns"][0] for traj in dataset]
    # cost_expert = [traj["cost_returns"][0] for traj in expert_trajs]
    # print(np.mean(rew), np.mean(rew_expert))
    # print(np.mean(cost), np.mean(cost_expert))
...
def process_sequence_dataset(dataset: dict, cost_reverse: bool = False):
    ...
    for i in trange(dataset["rewards"].shape[0], desc="Processing trajectories"):
        ...
        done = bool(dataset["terminals"][i]) or bool(dataset["timeouts"][i])
        data_["dones"].append(float(done))
...
class SequenceDataset(IterableDataset):
    ...
    def __init__(
        ...
        label_expert_trajectories(self.dataset, expert_pct, 20)
    ...
    def __prepare_sample(self, traj_idx, start_idx):
        ...
        # ← NEW: Extract raw rewards and dones (terminals | timeouts)
        rewards = traj["rewards"][start_idx: start_idx + self.seq_len]          # raw reward, unscaled
        dones = traj["dones"][start_idx: start_idx + self.seq_len]
        ...
        if states.shape[0] < self.seq_len:
            ...
            dones = pad_along_axis(dones, pad_to=self.seq_len)

        rewards = rewards[:, None]   # shape: [seq_len, 1]
        dones = dones[:, None]       # shape: [seq_len, 1]

        return states, actions, returns, cost_returns, time_steps, mask, episode_cost, costs, rewards, dones, traj["is_expert"]
