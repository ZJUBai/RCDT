...
from examples.configs.c2dt_configs import CDTQCTWeighted_DEFAULT_CONFIG, CDTQCTWeightedTrainConfig
from osrl.algorithms import CDT, CDTQCTWeightedTrainer, Critic
from osrl.common import SequenceDataset, load_datasets
from osrl.common.exp_util import auto_name, seed_all


@pyrallis.wrap()
def train(args: CDTQCTWeightedTrainConfig):
    # update config
    cfg, old_cfg = asdict(args), asdict(CDTQCTWeightedTrainConfig())
    differing_values = {key: cfg[key] for key in cfg.keys() if cfg[key] != old_cfg[key]}
    cfg = asdict(CDTQCTWeighted_DEFAULT_CONFIG[args.task]())
    cfg.update(differing_values)
    args = types.SimpleNamespace(**cfg)

    # setup logger
    default_cfg = asdict(CDTQCTWeighted_DEFAULT_CONFIG[args.task]())
    ...
    # data = load_datasets("datasets/", args.task)
    ...
    q_critic = Critic(state_dim=env.observation_space.shape[0], action_dim=env.action_space.shape[0], hidden_dim=args.embedding_dim).to(args.device)
    c_critic = Critic(state_dim=env.observation_space.shape[0], action_dim=env.action_space.shape[0], hidden_dim=args.embedding_dim).to(args.device)

    def checkpoint_fn():
        return {"model_state": model.state_dict(), "q_critic_state":q_critic.state_dict(), "c_critic_state":c_critic.state_dict()}

    logger.setup_checkpoint_fn(checkpoint_fn)

    # trainer
    trainer = CDTQCTWeightedTrainer(model,
                         q_critic=q_critic,
                         c_critic=c_critic,
                         env=env,
                         logger=logger,
    ...
    for step in trange(args.update_steps, desc="Training"):
        ...
        states, actions, returns, costs_return, time_steps, mask, episode_cost, costs, rewards, dones, is_expert = [
        ...
        if (step + 1) % args.eval_every == 0 or step == args.update_steps - 1:
            ...
            for target_return in args.target_returns:
                ...
                env.set_target_cost(cost_return)
                norm_ret, norm_cost = env.get_normalized_score(ret, cost)
                norm_best_ret, norm_best_cost = env.get_normalized_score(best_reward, best_cost)
                ...
                print(f"Step:{step+1}, Target:({reward_return},{cost_return}), Current:({ret},{cost}), Norm_Current:({norm_ret},{norm_cost}), Best:({norm_best_ret},{norm_best_cost})")
